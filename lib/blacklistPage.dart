...
class _AppList extends State<AppListWidget> {
          ...
          return ListView(
            children: [
              ...sortedAppList.map((app) => SelectorCardWidget(
                    ...
                    onChanged: (selected) {
                      if (app.name == null || app.packageName == null) return;
                      if (selected)
                        value.disabledApps
                            .upsert(App(app.name!, app.packageName!, selected));
                      else
                        value.disabledApps.remove(app.packageName!);
                    },
                    isActive: app.packageName != null &&
                        ((value.disabledApps.get(app.packageName!) ?? false)),
                  ))
            ],
          );
        },
